...
function EnhancedTableHead<T>(props: EnhancedTableProps<T>) {
  const { onSelectAllClick, headCells, order, orderBy, numSelected, rowCount, checkboxes, onRequestSort } =
    props;
...
function EnhancedTableToolbar(props: EnhancedTableToolbarProps) {
  const { numSelected, title, addHandler } = props;
...
export default function DataTable<T extends Record<string, any>>(props: DataTableProps<T>) {
  const { rows, headCells, checkboxes, title, selectHandler, editHandler, addHandler, deleteHandler, hideDelete } = props;
  ...
  const handleClick = (event: React.MouseEvent<unknown>, name: string, id: number, checkboxes?: boolean) => {
    if (checkboxes) {
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        <EnhancedTableToolbar numSelected={selected.length} title={title} addHandler={addHandler} />
        <TableContainer>
          <Table
            sx={{ minWidth: 750 }}
            ...
            <TableBody>
              {visibleRows?.map((row, index) => {
                ...
                return (
                  <TableRow
                    hover
                    onClick={(event) => handleClick(event, String(row?.name), Number(row?.id), checkboxes)}
                    role="checkbox"
                  ...
                  >
                    ...
                    <TableCell className='actions'>
                      <IconButton onClick={() => { selectHandler?.(row?.id); }}>
